...
import { cleanupApi, towersApi } from '@/lib/api'
...
const SIRKIT_OPTIONS: { value: string; label: string }[] = [
  { value: 'sirkit_1', label: 'Sirkit 1' },
  { value: 'sirkit_2', label: 'Sirkit 2' },
]
const SIRKIT_LABEL = Object.fromEntries(SIRKIT_OPTIONS.map((s) => [s.value, s.label]))
...
function CleanupDrawer({
  ...
  useEffect(() => {
    ...
    if (initial) {
      setForm({
        ...
        sirkit:     initial.sirkit ?? 'sirkit_1',
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!form.sirkit)  { toast.error('Pilih sirkit'); return }
    ...
    try {
      ...
      if (initial?.id) await cleanupApi.update(initial.id, payload)
      else             await cleanupApi.create(payload)
      toast.success(initial ? 'Cleanup diperbarui' : 'Cleanup ditambahkan')
      onSaved()
      onClose()
    } catch (err: any) {
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-app-border shrink-0">
          <h2 className="text-[15px] font-bold text-app-text">
            {initial ? 'Edit Cleanup' : 'Tambah Cleanup'}
          </h2>
          <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-app-bg text-app-muted">
            <X size={18} />
          </button>
        </div>
        <form id="cleanup-form" onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-5 py-5 space-y-4">
          <div>
            <SearchableSelect
              label="Penghantar *"
          ...
          <div>
            <SearchableSelect
              label="Sirkit *"
              placeholder="Pilih sirkit..."
              options={SIRKIT_OPTIONS}
              values={form.sirkit ? [form.sirkit] : []}
              onChange={(vals) => set('sirkit', vals[vals.length - 1] ?? '')}
              onClear={() => set('sirkit', '')}
            />
          </div>
          <div>
...
export default function CleanupPage() {
  ...
  const [sirkitFilter, setSirkitFilter] = useState<string[]>([])
  ...
  const fetchData = useCallback(async () => {
    setLoading(true)
    try {
      const res = await cleanupApi.getAll({
        page,
        limit,
        search: q || undefined,
        status: statusFilter.length ? statusFilter.join(',') : undefined,
        sirkit: sirkitFilter.length ? sirkitFilter.join(',') : undefined,
  ...
  }, [page, limit, q, statusFilter, sirkitFilter, jalurFilter, tglMulai, tglAkhir])

  useEffect(() => { fetchData() }, [fetchData])

  const hasActiveFilters = Boolean(statusFilter.length || sirkitFilter.length || jalurFilter.length || tglMulai || tglAkhir)
  ...
  async function confirmDelete() {
    ...
    setDeleting(true)
    try {
      await cleanupApi.delete(deleteId)
      toast.success('Cleanup dihapus')
      setDeleteId(null)
      fetchData()
    } catch (err: any) {
      const msg = err?.response?.data?.message
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-5 gap-3 flex-wrap">
        <h1 className="text-xl md:text-2xl font-bold text-app-text">Clean Up Isolator</h1>
        <button onClick={() => { setEditRow(null); setDrawerOpen(true) }} className="btn-primary">
          <Plus size={16} /> Tambah Cleanup
        </button>
      </div>
          ...
          {filterOpen && (
            <div className="absolute right-0 top-full mt-2 z-50 w-[min(420px,calc(100vw-2rem))] bg-white border border-[#E1E8EC] rounded-lg shadow-[0px_4px_8px_0px_rgba(28,28,28,0.15)]">
              ...
              <div className="max-h-[450px] overflow-y-auto">
                ...
                <div className="px-4 py-3 flex flex-col gap-2.5">
                  <span className="font-bold text-[14px] text-[#1C1C1C]">Sirkit</span>
                  <div className="flex flex-wrap gap-2">
                    {SIRKIT_OPTIONS.map(o => {
                      const active = sirkitFilter.includes(o.value)
                      return (
                        <button
                          key={o.value}
                          onClick={() => { setSirkitFilter(active ? sirkitFilter.filter(v => v !== o.value) : [...sirkitFilter, o.value]); setPage(1) }}
                          className={`px-3 py-1 rounded-full border text-[12px] font-medium transition-all ${active ? 'bg-[#076C9E] border-[#076C9E] text-white' : 'border-[#E1E8EC] text-[#5F737F] hover:border-[#076C9E]'}`}
                        >{o.label}</button>
                      )
                    })}
          ...
          <table className="data-table">
            ...
            <tbody>
              {loading
                ...
                : rows.length === 0
                  ? <tr><td colSpan={6}><EmptyState title="Belum ada Cleanup" /></td></tr>
                  : rows.map((row) => (
                    <tr key={row.id}>
                      <td className="font-semibold text-[13px] text-app-text">{row.tower?.nama ?? '—'}</td>
                      <td className="text-[12px] text-app-muted whitespace-nowrap">{SIRKIT_LABEL[row.sirkit] ?? row.sirkit}</td>
                      <td className="text-[12px] text-app-muted whitespace-nowrap">{fmtDate(row.tanggal)}</td>
                      <td className="max-w-[280px] truncate text-[12px] text-app-muted">{row.keterangan ?? '—'}</td>
      ...
      <div className="md:hidden space-y-3">
        {loading ? (
          ...
          <div className="card"><EmptyState title="Belum ada Cleanup" /></div>
        ) : rows.map((row) => (
            ...
            <div className="flex items-start justify-between gap-3 mb-2">
              <div className="min-w-0">
                ...
                <div className="text-[12px] text-app-muted">
                  {SIRKIT_LABEL[row.sirkit] ?? row.sirkit} · {fmtDate(row.tanggal)}
                </div>
              </div>
              <ActionMenu items={[
                { label: 'Edit', icon: <Pencil size={14} />, onClick: () => { setEditRow(row); setDrawerOpen(true) } },
      ...
      <ConfirmModal
        isOpen={!!deleteId}
        title="Hapus Cleanup?"
        message="Data Cleanup ini akan dihapus permanen."
