...
function CUIDrawer({ open, initial, onClose, onSaved }: { open: boolean; initial: any | null; onClose: () => void; onSaved: () => void }) {
  const user = getUser()
  const [towers, setTowers] = useState<any[]>([])
  const [form, setForm] = useState({
    towerId: '', tanggalWaktu: new Date().toISOString().slice(0, 16),
    teknisi: '', noSpk: '', temuan: '', hasil: '',
  ...
  useEffect(() => {
    if (open) {
      setForm(initial ? {
        ...
        temuan: initial.temuan ?? '', hasil: initial.hasil ?? '',
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true)
    try {
      const payload = { ...form, jenisGangguan: 'cui', tanggal: form.tanggalWaktu }
    ...
    <>
      ...
      <div className={`fixed top-0 right-0 bottom-0 z-50 flex flex-col bg-white shadow-2xl transition-transform duration-300 ${open ? 'translate-x-0' : 'translate-x-full'}`} style={{ width: 520 }}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-app-border shrink-0">
          <h2 className="text-[15px] font-bold text-app-text">{initial ? 'Edit Laporan CUI' : 'Tambah Laporan CUI'}</h2>
          <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-app-bg text-app-muted"><X size={18} /></button>
        </div>
        <form id="cui-form" onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
          <div>
...
export default function CUIPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true)
    try {
      const res = await laporanApi.getAll({ jenisGangguan: 'cui', page, limit, search: q || undefined })
      const p = res.data
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-5">
        <h1 className="text-2xl font-bold text-app-text">Climb Up Inspection</h1>
        <button onClick={() => { setEditRow(null); setDrawerOpen(true) }} className="btn-primary">
          <Plus size={16} /> Tambah Laporan CUI
        </button>
      </div>
      ...
      <div className="card overflow-hidden">
        <div className="overflow-x-auto">
          <table className="data-table">
            ...
            <tbody>
              {loading ? <SkeletonRow cols={8} rows={limit} />
              : rows.length === 0 ? <tr><td colSpan={8}><EmptyState title="Belum ada laporan CUI" /></td></tr>
              : rows.map((row) => (
                <tr key={row.id}>
                  ...
                  <td>{row.teknisi ?? '—'}</td>
                  <td className="max-w-[180px] truncate text-[12px] text-app-muted">{row.temuan ?? '—'}</td>
                  <td className="max-w-[180px] truncate text-[12px] text-app-muted">{row.hasil ?? '—'}</td>
      ...
      <ConfirmModal isOpen={!!deleteId} title="Hapus Laporan CUI?" message="Laporan ini akan dihapus permanen." onConfirm={confirmDelete} onCancel={() => setDeleteId(null)} loading={deleting} confirmLabel="Ya, Hapus" />
    </>
  )
}
